...
BEGIN_DATADESC(C_AmbientFMOD)

DEFINE_FIELD(m_nCurTime, FIELD_INTEGER),
...
void C_AmbientFMOD::ClientThink( void )
{
	if ( m_bInit && m_pChannel != nullptr )
	{
		...
		if (!m_bRestoreTime)
		{
			m_pChannel->getPosition(&m_nCurTime, FMOD_TIMEUNIT_MS);
		}
		else
		{
			m_pChannel->setPosition(m_nCurTime, FMOD_TIMEUNIT_MS);
			m_bRestoreTime = false;
		}
	}
}
...
bool C_AmbientFMOD::SetSound()
{
	...
	if ( m_pChannel ) 
	{
		...
		if ( !m_bGlobal )
		{
			...
			if (m_hSoundSource)
			{
				entPos = m_hSoundSource->GetAbsOrigin();
			}
