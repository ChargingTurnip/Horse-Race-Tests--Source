...
#include "horse_race_test/logic_horserace.h"
...
CON_COMMAND_F(selecthorse, "Select horse", FCVAR_NONE)
{
	CBasePlayer* pPlayer = UTIL_PlayerByIndex(1);
	if (pPlayer)
	{
		const char* szHorse = args[1];

		CHandle< CLogicHorseRace > pLogic = NULL;
		pLogic = (CLogicHorseRace*)gEntList.FindEntityByClassname(pLogic, "logic_horserace");
		if (pLogic)
		{
			pLogic->SetHorse(szHorse);
		}
	}
}

CON_COMMAND_F(clearhorses, "Clear Horses", FCVAR_NONE)
{
	CBasePlayer* pPlayer = UTIL_PlayerByIndex(1);
	if (pPlayer)
	{
...
CON_COMMAND_F(readyhorses, "Ready Horses", FCVAR_NONE)
{
	CBasePlayer* pPlayer = UTIL_PlayerByIndex(1);
	if (pPlayer)
	{
...
CON_COMMAND_F(togglehorseselect, "Toggle Horse Select", FCVAR_NONE)
{
	CSingleUserRecipientFilter user(UTIL_GetLocalPlayer());
	UserMessageBegin(user, "ToggleHorseSelect");
	WRITE_BYTE(0),
	MessageEnd();
}
